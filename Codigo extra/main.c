...
int main()
{
    void *buffer = malloc (6*sizeof(int) //vari�veis de controle
                           + 1*sizeof (agenda) //aloca pelo menos o espa�o de um contato na agenda
                           + 30*sizeof(char));
    void *bufferTemp;
    void *enderecoExclusao;
    int *menu;
    int *quantidadeDeContatos; //quantidade de contatos que est�o na agenda
    ...
    if (buffer == NULL) {   //verifica se a aloca��o foi feita com sucesso
        printf ("NAO FOI POSSIVEL ALOCAR MEMORIA!\n");
        exit (1);
    }

    menu = buffer; //primeira posi��o serve para controle do menu
    quantidadeDeContatos = buffer+1*sizeof(int); //segunda posi��o de inteiros do buffer vai guardar quantos contatos temos na agenda
    *quantidadeDeContatos = 0; //no come�o n�o temos nenhum contato
    ...
    do {
        *quantidadeDeContatosAuxiliar = *quantidadeDeContatos;
        if (*(quantidadeDeContatos) >= 2) {
            //chama fun��o para ordenar com Merge Sort
            bufferTemp = calloc (6*sizeof(int)  //alocando em outro posi��o, depois tem que transferir pra alocar no buffer
        ...
        if (*(menu) == 1) {
            ...
            *(quantidadeDeContatosAuxiliar) = *(quantidadeDeContatos);
            ...
            menu = buffer;     //atualiza os ponteiros com os novos endere�os de mem�ria
            ...
            if (enderecoExclusao == NULL) {
                printf ("Contato nao localizado para exclusao!\n");
                    ...
                    /*
                        implementar aqui a transfer�ncia do pr�ximo contato no contato que foi exclu�do
                    */
            ...
            free (buffer);  //libera a mem�ria e encerra o programa
            exit(0);
        }
    } while (1);
    return 0;
...
void *Excluir (void *PrimeiroContato, char *nomeParaExcluir, int *QuantidadeDeContatos) {
    while (*(QuantidadeDeContatos) > 0) {
        if (strstr(nomeParaExcluir, PrimeiroContato) != NULL) {    //achou o contato para exclus�o
            PrimeiroContato = (PrimeiroContato + sizeof(agenda)); //sobrep�e o pr�ximo contato no contato exclu�do
            return PrimeiroContato;   //exclu�do com sucesso
        }
        PrimeiroContato += sizeof(agenda);
        (*(QuantidadeDeContatos))--;
    }
    return NULL; //contato n�o localizado para exclus�o
}
